...
import * as WhaleDivingInvariant from '@deepseek-ai/dsh-client-ui-whale-diving/invariant'
...
describe('invariant companion', () => {
  it('reserves package ownership with an empty installer', async () => {
    ...
    await expect(ctx.plugin(WhaleDivingInvariant).await()).resolves.toBeDefined()
  })

  it('has an empty node half', async () => {
    const { apply } = await import('@deepseek-ai/dsh-client-ui-whale-diving')
